...
```youtube
https://www.youtube.com/watch?v=W68-v6FSwaQ
Install Ubuntu alongside Windows — full step-by-step walkthrough
```

> Read the written steps below alongside the video — the video is great for *seeing* what each screen looks like, the text is great for *not having to scrub back* when you're stuck on a specific step.
...
```mermaid
flowchart TD
    A([📦 Get a USB pendrive<br/>8 GB or more]) --> B([⬇️ Download Ubuntu ISO])
    B --> C([⬇️ Download Rufus])
    C --> D([💾 Flash USB with Rufus])
    D --> E([🗂️ Backup your files])
...
> ⏱ **Time:** about **10–20 minutes** once you have the USB drive ready.
